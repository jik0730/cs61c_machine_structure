...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
     #pragma omp parallel
	{
		...
		for(int i=0; i<ARRAY_SIZE; i = i + total_threads) {
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
          #pragma omp parallel
	{
		int total_threads = omp_get_num_threads();
		int chunk_size = 0;
		if (ARRAY_SIZE % total_threads == 0) {
			chunk_size = ARRAY_SIZE / total_threads;
		} else {
			chunk_size = ARRAY_SIZE / total_threads + 1;
		}
		for(int j=0; j<chunk_size; j++) {
			int k = omp_get_thread_num()*chunk_size;
			if (j+k >= ARRAY_SIZE)
				continue;
			z[j+k] = x[j+k] + y[j+k];
		}
